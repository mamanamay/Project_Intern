...
const dataPath = path.resolve('data/cv.json');
...
export async function GET() {
  try {
    ...
    console.error('API Error (GET /api/cv):', err, 'Path was:', dataPath);
    return json({ error: 'Failed to read data', path: dataPath, details: err.message }, { status: 500 });
  }
}
